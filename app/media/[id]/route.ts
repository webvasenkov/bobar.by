...
export const GET = handleApi(async request => {
  ...
  const published = await database.prepare("SELECT id FROM portfolio_projects WHERE published = 1 AND (desktop_image = ? OR mobile_image = ?) LIMIT 1").bind(`/media/${id}`, `/media/${id}`).first();
